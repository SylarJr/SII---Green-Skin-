...
async function cargarMateriasProfesor() {
    ...
    try {
        // 2. Pedir los horarios a Supabase usando la función SQL
...
function renderizarHorarios(materias) {
    ...
    materias.forEach(materia => {
        ...
        // Estilos dinámicos basados en el turno
        ...
        const tarjetaHTML = `
            <div class="materia-card-profesor" data-turno="${turno}" style="background: white; border-radius: 12px; border-left: 6px solid ${borderLeft}; padding: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); transition: 0.3s;">
                ...
                <div style="font-size: 0.85rem; color: #666; display: flex; flex-direction: column; gap: 6px;">
        ...
        const filaHTML = `
            <tr class="fila-materia-tabla" data-turno="${turno}">
                ...
                <td>${materia.dias} (${materia.hora_inicio} - ${materia.hora_fin})</td>
            </tr>
        `;
...
// Lógica de los botones de filtrado (Matutino/Vespertino)
